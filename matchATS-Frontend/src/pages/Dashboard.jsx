...
const Dashboard = () => {
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-white p-10 rounded-3xl shadow-xl border border-gray-100 flex flex-col items-center max-w-sm w-full">
          ...
          <p className="mt-2 text-gray-500">
            Our AI is matching {files.length} resumes against your
            requirements...
          </p>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-8">
          <button
            onClick={resetAll}
            className="mb-3 flex items-center gap-2 text-indigo-600 font-semibold hover:text-indigo-800 transition-colors"
          >
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 flex flex-col items-center">
        {/* Hero Section */}
        <div className="text-center mb-12">
          ...
          <p className="text-slate-600 text-lg max-w-2xl mx-auto">
            Upload candidate resumes and paste your job description. Our AI will
            rank them based on semantic fit.
          </p>
        </div>
        ...
        <div className="w-full bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="p-8 bg-slate-50/30 flex flex-col">
            <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-4 uppercase tracking-wider">
              <Upload className="w-4 h-4 text-indigo-500" /> Upload Resumes
            </label>
              ...
              <div className="mt-4 p-3 bg-white rounded-xl border border-slate-200">
                ...
                <div className="max-h-24 overflow-y-auto space-y-1">
                  {files.slice(0, 3).map((file, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-2 text-xs text-slate-600 truncate"
                    >
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-10 w-full max-w-md">
          ...
          <button
            onClick={resetAll}
            className="w-full sm:w-auto px-6 py-4 bg-white text-slate-600 rounded-2xl font-semibold border border-slate-200 hover:bg-red-500 hover:text-white transition-all"
          >
            Reset
          </button>
        </div>
